...
数字 tab ID 只在一个标签页生命周期内有效，并由持久化的 `tabFence` 保护；生命周期事件不能授权对无关复用 ID 的操作。完整 Handle 还包含由 URL、`performance.timeOrigin` 和每个文档独有 Token 组成的 `incarnation`。snapshot ref 是来源 document 内的地址：同一元素在文档存活期内始终是同一个编号，即使某次 observation 已被淘汰，也能由页面内的文档注册表解析（结果标注 resolvedBy: document_registry），而不是直接失败。DOM-CUA node id 仍是来源 document 内有界的 live observation：标题/焦点变化、后续 observation 和无关 DOM churn 会保留仍满足语义的原 node；原 node 被移除时，最多只允许一次唯一、强等价的同 document rebind。同一文档只有标题变化时不会使完整 Handle 失效；导航、reload 和 document replacement 会以 `BROWSER_DOCUMENT_CHANGED` 使旧 observation 失效，Tab 关闭和 tab fence 变化分别返回 `BROWSER_TAB_CLOSED` 和 `BROWSER_TAB_FENCE_CHANGED`；dialog、file chooser 和 Network request-loader 映射也随之失效。`navigate(wait: false)`、`back`、`forward` 或 `reload` 的返回 Tab 明确标记为 `transitionPending`，其 handle 省略不稳定的 URL/title 和 incarnation，调用方必须先等待并重新观察后再进行 document-bound 操作；读取 Network response body 时，必须同时使用当前 listing 中匹配的 `requestId` 和 `loaderId`。用户 Tab 的只读页面读取会重新观察当前文档并在变化时重试一次，持续变化时返回 `BROWSER_PAGE_CHANGING`。有副作用的页面操作继续在执行前后检查文档身份：页面 action 已确认返回且 post-action identity 可读取时，即使随后发生页面转换也保持成功；只有注入结果丢失或 post-action identity 无法验证时才标记为不确定且不可自动重试。受限的 `about:blank` 可以保留 Tab 身份但没有文档 incarnation，页面操作前应先导航到可注入脚本的 URL。
...
本发行版是**能力层**：它只理解页面的*结构*，以及如何安全地观察和操作页面；它刻意不理解任何产品的字段名、状态词表和布局。站点相关的处理属于调用方 Skill，由 Skill 用通用原语组合实现。

| 关注点 | 归属 | 例子 |
| --- | --- | --- |
| 页面结构 | 能力层 | landmark、标题层级、页面主对象、工具区/样板区降权、次要区域、Key actions、截断标记 |
| 身份、安全、生命周期 | 能力层 | `browserId` / `tabFence` / `incarnation` / `snapshotId`、标签页所有权与租约、清理、能力声明、扩展自加载、不确定副作用不自动重放 |
| 通用原语 | 能力层 | wait 状态（含调用方给出的终态文本集合）、`tail` 与 `logMatch` 的提取范围、`evaluate`、CDP、Console、Network、Dialog、Upload、Download、Clipboard |
| 站点无关数据 | 能力层 | 只取渲染文本的 Page Map、有界的 `label: value` 对、ARIA status 区域、凭据类标签行脱敏 |
| 站点 DOM 形状 | 个性化层（Skill） | 选择器、字段名、状态词表、布局细节 |
| 站点流程 | 个性化层（Skill） | 步骤顺序、触发去重、重试策略、证据契约、汇报格式 |
| 领域解析 | 个性化层（Skill） | CI 页面的 build/revision/branch/node/duration/result，以及其他产品页面的对应字段 |

规则：

1. 一个站点事实如果可以用调用方给出的字面量或调用方自己写的页面脚本承载，就不进入插件。
2. 只有当一个站点事实能上升为**结构性原则**、且对不相关的页面原型同样成立时，才允许进入插件——例如"搜索、页头、页脚、面包屑、分页、侧边栏、逐行历史控件属于工具区或列表，永远不是页面主对象"。产品名、站点 URL、站点选择器永远不允许。
3. 站点逻辑放在拥有该流程的 Skill 里：只读、有界、自包含，并在该流程旁边记录它的字段契约。
4. 当插件对某站点的结构输出不正确时，修的是通用排序或降噪规则并补充原型用例，而不是加站点特例。
5. 测试遵循同一条分界：插件测试在合成原型（无 landmark 应用壳、`<main>` 应用、业务页面、日志面板、敌意页面）上断言通用不变量；领域字段由拥有该流程的 Skill 断言。
6. 能力声明是强制点：插件通过 `waitTerminalStates`、`extractLogMatch` 这类原语暴露能力，从而永远不需要自己识别产品的终态字符串；运行时陈旧会被拒绝，而不是静默降级。

代码评审中，向 `extension/`、`bridge/` 或宿主适配器新增产品名、站点 URL、站点选择器或领域字段名，默认属于超出范围；应把该逻辑移到调用方 Skill。
