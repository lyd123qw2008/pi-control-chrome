...
- [ ] **P0** Chrome 扩展首次安装是唯一的浏览器授权边界
- [ ] **P0** 本地 Bridge 默认只监听 `127.0.0.1`
- [ ] **P0** Bridge 使用随机 Token，拒绝普通网页请求
- [ ] **P0** Token 不进入模型上下文、不写入日志
- [ ] **P0** 读操作和写操作分级
- [ ] **P0** 表单提交、发送消息、发帖、购买、删除、上传、权限修改前确认
- [ ] **P0** 密码、OTP、API Key、支付数据、Cookie、Storage 发送前确认
- [ ] **P0** 页面内容不能覆盖用户指令或授予额外权限
- [ ] **P1** 插件 Popup 提供 Pause/Resume/Disconnect
- [ ] **P1** Pi `/chrome pause` 立即暂停工具执行
- [ ] **P1** Pi `/chrome revoke` 断开当前会话
- [ ] **P1** 浏览器操作审计日志
- [ ] **P1** 当前 session 和其他 Pi session 隔离
- [ ] **P2** 每个站点/域名的细粒度权限
...
8. 是否接受 Chrome 扩展首次安装/配对一次，后续不再逐次授权？
9. Cookie、Storage、密码、OTP、上传、表单提交是否都保留 action-time 确认？
