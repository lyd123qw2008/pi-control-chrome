...
- Returns bounded semantic page state by default: visible/actionable snapshot nodes, 20,000-character/200-node snapshot and DOM-CUA budgets, and a shared 12,000-character extract budget. `browser_snapshot` keeps refs and `snapshotId` while Pi/DSH projections omit duplicate raw accessibility and frameTree data; when tab metadata already carries title and URL, projections do not repeat them inside page results, and full page text is reserved for `browser_extract` when no semantic state is available.
