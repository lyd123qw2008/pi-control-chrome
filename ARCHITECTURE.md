...
### Cache-clean host exposure

The DSH adapter supports a `progressive` exposure mode in addition to its legacy `lazy-full` catalog. Progressive mode registers only three fixed core tools — `browser_capabilities`, `browser_call` and `browser_status` — from plugin startup. It returns capability summaries and one operation schema through tool results, then dispatches the validated operation through the same executor used by raw `browser_*` tools; confirmation-protected lifecycle operations carry `arguments.confirmed: true` only after explicit user confirmation. Capability discovery never changes the model-visible tool set, and it never contacts the Bridge by itself. The Pi adapter keeps its existing raw-tool activation behavior until it explicitly adopts the facade. This host-level boundary is intentionally separate from the extension capability revision: a stale extension is still rejected at the existing Bridge/request gates.
