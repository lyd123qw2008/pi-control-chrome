...
```powershell
corepack pnpm --dir <DSH_HOME>/profiles/web add @lyd123qw2008/dsh-tool-control-chrome@0.8.1
```
...
{
  "dependencies": {
    "@lyd123qw2008/dsh-tool-control-chrome": "0.8.1"
  }
}
```
...
`browser_restart` is a model-facing lifecycle tool that requires `{ "confirmed": true }` after an explicit user confirmation. It restarts only the shared local Bridge through the active DSH `BrowserBridgeClient.restart()` implementation; it does not restart DSH or Chrome/Edge, close tabs, or clear login state. The tool waits for the extension to reconnect when possible, returns `state: "bridge_only"` when the Bridge is healthy but the extension remains offline, and invalidates the session's previously acknowledged target route. After a successful restart, treat every previous tab handle, snapshot ref, DOM-CUA node, and document incarnation as stale: call `browser_status` with the intended `browserId` to acknowledge the new connection, then call `browser_tabs` and take fresh observations before page actions. Legacy Bridges without cooperative restart capability are left untouched.
