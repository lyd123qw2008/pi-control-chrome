...
describe('DSH browser tool catalog', () => {
  ...
  it('projects bounded snapshots and preserves refs when page text is large', async () => {
    ...
    expect(snapshot).toMatchObject({ snapshot: { snapshotId: 'snapshot-1', nodeCount: 1 } })
    expect((snapshot as { snapshot: Record<string, unknown> }).snapshot.truncated).toBeUndefined()
  ...
  it('projects accessibility and network specializations', async () => {
    ...
    expect(accessibility).toEqual({ snapshotId: 'snapshot-1', mode: 'full', state: '', nodeCount: 0, charCount: 0 })
