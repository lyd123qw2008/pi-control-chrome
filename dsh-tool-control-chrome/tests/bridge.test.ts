...
it('resolveConfig rejects non-loopback Bridge hosts and accepts defaults', () => {
  ...
  expect(defaults.exposureMode).toBe('lazy-full')
  expect(resolveConfig({ exposureMode: 'progressive' }).exposureMode).toBe('progressive')
  expect(() => resolveConfig({ exposureMode: 'invalid' as never })).toThrow(/exposureMode/)
