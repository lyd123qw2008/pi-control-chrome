...
- `values` 只发布页面自己渲染的 `label: value` 对（上限 12），不做字段名归一化、不推断领域字段。除凭据词汇外，还有五条**结构**约束（与站点无关）：
  - label ≥ 2 字符——单字符单元格是坐标或布局占位（真实构建页的构建历史表用 `x` 占位，曾发布成 `x: x`），不是字段名；
  ...
  - **表头行不是数据**：整行都是表头单元格（`th` / `role=columnheader`）或位于 `<thead>` 内的行会被跳过——列名不是"标签: 值"（真实构建页的图表表头 `W | Description | %` 曾发布成 `w: Description · %`）；只有"首个单元格是表头、其余是数据"的行（`<th>Label</th><td>Value</td>`）仍按声明对发布。
