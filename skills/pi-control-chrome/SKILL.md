...
- `browser_wait`: wait for page load, URL, text, text disappearance, or an element to become visible, hidden, or enabled. Use `text` for page text and `target` for element conditions; AX targets require the matching `snapshotId`. `hidden` succeeds when no visible match exists; `visible` and `enabled` require one visible match and reject multiple visible matches. Visible/enabled waits apply an explicit index after visibility filtering.
