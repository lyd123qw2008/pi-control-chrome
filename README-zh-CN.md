...
- 原生 CDP；
- Runtime.evaluate；
- Console、Network、Dialog、Upload、Download；Console 和 Network 每次读取最多返回 200 条、20,000 个序列化字符。Console 支持 `only: "errors"` 过滤和 `since`/`nextSince` 增量游标，并包含 `pageerror` 事件；
- `browser_probe_interaction` 对一次明确的页面动作返回前后 document identity、动作确认、动作后的 Console 错误、settle 状态和目标 postState，用于 UI 回归排查；不自动重放不确定的副作用；
- Chrome/Edge 页面能力检测；
- `browser_probe_interaction` 单次交互诊断，关联动作确认、前后 document identity、动作后 Console/pageerror、settle 和目标状态；
- 语义元素定位和页面状态等待。
...
仓库还包含独立的 [`@lyd123qw2008/dsh-tool-control-chrome`](./dsh-tool-control-chrome/README.md) 包。默认 `exposureMode: lazy-full` 且 `lazyTools: true` 时，插件只注册 `pi-control-chrome` Skill 的名称和描述；Skill 成功加载后，把完整的 44 个 raw `browser_*` 工具注册到当前 Agent，并在该 Agent session 的连续 turn 中保持激活。设置 `exposureMode: progressive` 时，插件从启动阶段只固定注册三个核心 facade 工具：`browser_capabilities`、`browser_call`、`browser_status`；44 个 raw schema 不会在会话中途加入，能力组和单 operation schema 通过 tool result/message 按需返回，`browser_call` 使用 operation registry 和共享 executor。需要确认的生命周期 operation 通过 `arguments.confirmed: true` 调用，且只能在用户明确确认后使用；整个过程不会改变模型可见的工具集合。按 Codex 默认生命周期，turn 结束时宿主关闭未标记的 Agent 临时 Tab、release claimed user Tab、detach debugger lease，但不停止 Bridge、移除 progressive facade 或重新加载 Skill。模型需要保留页面时，lazy-full 应在当前 turn 直接调用 `browser_mark_handoff` 或 `browser_mark_deliverable`；progressive 应发现并通过 `browser_call` 派发同名 operation，下一 turn 仍需保留时重新标记。只有用户明确要求立即关闭临时 Tab、释放 claim 或清理浏览器任务时，才执行 `browser_cleanup`；progressive 通过 `browser_call` 传入 `arguments.confirmed: true`，lazy-full 可调用直接 raw tool，它会保留工具和健康 Bridge。只有在用户明确决定恢复扩展运行时之后，才把 `recoverStale: true` 传给 `browser_cleanup`；该选项只忘记未知运行时的 ownership 记录，不关闭对应 Tab，并在 `recovered` 中返回 Tab id 供人工检查。`browser_context_reset` 是单独的显式用户请求操作，用于 finalize 资源；progressive 通过 `browser_call` 传入确认参数，lazy-full 会停用惰性 raw 工具，progressive 会保留三个核心 facade。Agent disposal 和插件关闭会重试最终清理；清理恢复失败时，复用同一 session ID 的替代 Agent 会保持阻塞，直到清理成功。设置 `lazyTools: false` 可保留 lazy-full 插件加载即显示 raw 工具的兼容行为。Pi 使用同一批原生工具定义和 active-tool 隐藏机制。DSH 还提供人工使用的 `/chrome status`、`/chrome targets`、`/chrome profile [browserId]`、`/chrome connect`、`/chrome disconnect`、`/chrome doctor`、`/chrome restart` 和 `/chrome tabs` 命令；这些命令不替代 Skill 激活。
...
然后把它的 `cordis.patch.yml` 中的 `insert` 条目合并到现有 `cordis.patch.yml`（用 `dsh plugin --profile web add` 安装时会自动应用该 bundle patch），不要覆盖其他 patch 条目，并把 Bridge 配置放到 `<DSH_HOME>/settings.yaml` 的 `control-chrome` 命名空间。DSH Profile 应包含标准的 `@deepseek-ai/dsh-skill` 服务，以便使用运行时 Skill 注册表和 `skill` 工具；浏览器插件本身不强制注入该可选服务。DSH 包复用本项目的 Bridge 和 Manifest V3 扩展，不会自动安装浏览器扩展，不读取 Chrome Profile 文件，也不会把 Bridge 暴露到 loopback 之外。
